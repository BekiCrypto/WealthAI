...
stop (invalidation), and target -- the closest thing in the app to spec
step 9's "give me the highest-probability Gold setup" -- instead of leaving
"72% bullish" as an abstract call nobody can actually act on or disprove.

Direction and conviction come from the Intelligence Score (macro + technical
+ sentiment + geopolitical, spec step 7); the actual entry/stop/target
*levels* come from pure price structure (trend, RSI, support/resistance,
ATR), since macro conviction alone doesn't tell you where to place a stop.

When the score is too close to 50/50 or its confidence is Low, this
...
OVERBOUGHT = 70.0
OVERSOLD = 30.0
MIN_RISK_REWARD = 1.2  # below this, the stop distance isn't justified by the target -- not a setup worth showing
...
def generate_trade_setup(symbol: str, score: ScoreBreakdown, snapshot: TechnicalSnapshot) -> dict:
    ...
    if direction == "long":
        ...
        if stop >= entry:
            stop = entry - 1.5 * atr
        risk = entry - stop
        target = resistance if (resistance is not None and resistance > entry) else entry + 2 * risk
        ...
        if stop <= entry:
            stop = entry + 1.5 * atr
        risk = stop - entry
        target = support if (support is not None and support < entry) else entry - 2 * risk
    ...
    if risk_reward is not None and risk_reward < MIN_RISK_REWARD:
        return _no_setup(
            ...
            f"risk/reward is only {risk_reward}:1 -- the stop is too far from entry relative to "
            f"the target to be worth taking, so no setup is offered.",
        )
    ...
    return {
        ...
        "target_pct_from_entry": round((target - entry) / entry * 100, 2),
